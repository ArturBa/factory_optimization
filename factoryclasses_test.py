...
class Factory:
    ...
    def run(self):
        ...
        # 2. not enough material for full work but enough for requirements
            ...
            spare_material = self.material - (self.req_big_parts * self.big_machine.mat_required + self.req_small_parts * self.small_machine.mat_required)
            ...
            if self.big_machine.prep_time + ceil(self.req_big_parts/self.big_machine.machine_count) * big_real_runtime > 8:
                big_requirements_time = 2 * self.big_machine.prep_time + ceil(self.req_big_parts/self.big_machine.machine_count) * big_real_runtime
            ## big done on 1 shift
            else:
                big_requirements_time = self.big_machine.prep_time + ceil(self.req_big_parts/self.big_machine.machine_count) * big_real_runtime
            ## small done on 2 shifts
            if self.small_machine.prep_time + ceil(self.req_small_parts/self.small_machine.machine_count) * small_real_runtime > 8:
                small_requirements_time = 2 * self.small_machine.prep_time + ceil(self.req_small_parts/self.small_machine.machine_count) * small_real_runtime
            ## small done on 1 shift
            else:
                small_requirements_time = self.small_machine.prep_time + ceil(self.req_small_parts/self.small_machine.machine_count) * small_real_runtime
            ...
            print(f'small elapsed time = {small_elapsed_time}\n'
            +f'big elapsed time = {big_elapsed_time}\n'
            +f'spare material = {spare_material}\n'
            +f'spare_small parts = {spare_small_parts}\n'
            +f'spare_big parts = {spare_big_parts}\n')
            ...
            while spare_material >= self.big_machine.mat_required or spare_material >= self.small_machine.mat_required:
                ...
                if small_elapsed_time < big_elapsed_time:
                    ...
                    if small_first_run  and self.req_small_parts % self.small_machine.machine_count !=0:
                        ...
                        if spare_material >= self.small_machine.mat_required * (self.small_machine.machine_count -
                            ...
                            spare_material -= self.small_machine.mat_required * (self.small_machine.machine_count -
                                                                                 self.req_small_parts % self.small_machine.machine_count)
                            spare_small_parts += (self.small_machine.machine_count - self.req_small_parts % self.small_machine.machine_count)
                            small_first_run = False
                            print(f'small elapsed time = {small_elapsed_time}\n'
                                  + f'big elapsed time = {big_elapsed_time}\n'
                                  + f'spare material = {spare_material}\n'
                                  + f'spare small parts = {spare_small_parts}\n'
                                  + f'spare_big parts = {spare_big_parts}\n')
                        # not enough material
                        else:
                            print('not enough material')
                            spare_material -= self.small_machine.mat_required * (
                                        spare_material // self.small_machine.mat_required)
                    ...
                    # regular cycle
                        ...
                        print('small regular cycle')
                        # not enough material for all machines
                        if spare_material // self.small_machine.mat_required < self.small_machine.machine_count:
                            print('not enough material for all machines')
                            spare_small_parts += spare_material // self.small_machine.mat_required
                            spare_material -= (spare_material // self.small_machine.mat_required) * self.small_machine.mat_required
                            # workers' shift change
                            if small_elapsed_time < 8 and small_elapsed_time + small_real_runtime > 8:
                                print('workers shift change')
                                small_elapsed_time += (small_real_runtime + self.small_machine.prep_time)
                                print(f'small elapsed time = {small_elapsed_time}\n'
                                      + f'big elapsed time = {big_elapsed_time}\n'
                                      + f'spare material = {spare_material}\n'
                                      + f'spare_small parts = {small_parts}\n'
                                      + f'spare_big parts = {spare_big_parts}\n')
                            else:
                        ...
                        # enough material for all machines
                            ...
                            print('enough material for all machines')
                ...
                # big finished first
                    ...
                    if big_first_run and self.req_big_parts % self.big_machine.machine_count !=0:
                        ...
                        if spare_material >= self.big_machine.mat_required * (self.big_machine.machine_count -
                            ...
                            spare_material -= self.big_machine.mat_required * (self.big_machine.machine_count -
                                                                               self.req_big_parts % self.big_machine.machine_count)
                            spare_big_parts += (self.big_machine.machine_count - self.req_big_parts % self.big_machine.machine_count)
                            big_first_run = False
                            print(f'small elapsed time = {small_elapsed_time}\n'
                                  + f'big elapsed time = {big_elapsed_time}\n'
                                  + f'spare material = {spare_material}\n'
                            ...
                            spare_material -= self.big_machine.mat_required * (
                                        spare_material // self.big_machine.mat_required)
                    ...
                    # regular cycle
                        ...
                        print('big regular cycle')
                        # not enough material for all machines
                        if spare_material // self.big_machine.mat_required < self.big_machine.machine_count:
                            print('not enough material for all machines')
                            spare_big_parts += spare_material // self.big_machine.mat_required
                            spare_material -= (
                                                          spare_material // self.big_machine.mat_required) * self.big_machine.mat_required
                            # workers' shift change
            ...
            print(f'small parts = {small_parts}\n'+ f'big parts = {big_parts}\n')
